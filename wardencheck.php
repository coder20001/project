...
$wardenemail = $conn->real_escape_string($_REQUEST['useremail']); // Fixed variable name
$location = $conn->real_escape_string($_REQUEST['house']); // Ensure you have 'house' in your form
$wardNumber = $conn->real_escape_string($_REQUEST['wardnumber']); // Adjust according to your form input
$role = $conn->real_escape_string($_REQUEST['role']); // New role parameter
// Prepare the SQL statement
$stmt = $conn->prepare("INSERT INTO user (UserFName, UserLName, UserEmail, UserPassword, ResidenceLocation, WardenNumber, Role) 
 VALUES (?, ?, ?, ?, ?, ?, ?)");
//error handling
if($stmt === false){
...
$hashed_warden = password_hash($pass, PASSWORD_DEFAULT);


// Check that you have the right number of parameters
if ($stmt) {
    // Bind parameters: "sssssss" corresponds to the types
    $stmt->bind_param("sssssss", $userf, $userl, $wardenemail, $hashed_warden, $location, $wardNumber, $role);
