...
// Assuming you're fetching data for a specific user with id=1
$user_id = 1;
...
$sql = "SELECT 
    ...
    JOIN 
        ...
        // Add filtering by Student Number;";
...
function getReportedFaults() {
   ...
   /*  $sql = "SELECT 
            ...
            mt.TicketID AS work_order, 
            mt.status, 
            mt.CategoryID AS work_order_title, 
            r.ResidenceLocation AS residence_location, 
            CONCAT(u.UserFName, ' ', u.UserLName) AS reported_by
        FROM 
            maintenanceticket mt
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        <td><button class=\"view-details-btn\" onclick=\"location.href='maintain.php?id=" . urlencode($row['TicketID']) . "';\">
                Update Ticket </button></td>
